...
interface AppSettingsManagerProps {
  ...
  initialTab?: 'general' | 'logo' | 'running_text' | 'security';
}
...
export const AppSettingsManager: React.FC<AppSettingsManagerProps> = ({ onSettingsSaved, initialTab }) => {
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'logo' | 'running_text' | 'security'>(initialTab || 'general');
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 dark:border-slate-700 pb-2 overflow-x-auto no-scrollbar">
        ...
        >
          ...
          <span>Keamanan & Anti-Cheat</span>
          <span className={`px-1.5 py-0.2 rounded-full text-[10px] font-bold ${
            settings.antiCheatSecurityPopupsEnabled !== false
              ? (activeTab === 'security' ? 'bg-emerald-500/80 text-white' : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-300')
              : (activeTab === 'security' ? 'bg-slate-500/80 text-white' : 'bg-slate-200 text-slate-600 dark:bg-slate-800 dark:text-slate-400')
          }`}>
            Pop-up: {settings.antiCheatSecurityPopupsEnabled !== false ? 'ON' : 'OFF'}
          </span>
        </button>
      </div>
      ...
      {/* TAB CONTENT: Keamanan & Anti-Cheat */}
      {activeTab === 'security' && (
        <div className="space-y-6">
          {/* Header Description */}
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="space-y-1">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 text-xs font-bold border border-emerald-500/30">
                  ...
                  <span>Sistem Proteksi Integritas & Anti-Rekayasa</span>
                </div>
                <h3 className="text-lg font-bold text-white tracking-tight">
                  Pengaturan Keamanan Presensi & Anti-Cheat
                </h3>
                <p className="text-xs text-slate-300 max-w-2xl leading-relaxed">
                  Konfigurasikan sistem proteksi dari manipulasi koordinat GPS (Fake GPS), pintasan alat pengembang (DevTools/F12), proteksi klik kanan antarmuka, serta switch pop-up peringatan keamanan untuk pengguna.
                </p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <button
                  type="button"
                  onClick={() => AntiCheatSecurityService.getInstance().triggerTestPopup(false)}
                  className="px-3.5 py-2 rounded-xl text-xs font-semibold bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 transition flex items-center gap-2 cursor-pointer shadow-xs"
                  title="Uji coba tampilan pop-up sesuai status switch saat ini"
                >
                  <Bell className="w-3.5 h-3.5 text-amber-400" />
                  <span>Uji Coba Pop-up</span>
                </button>
              </div>
            </div>
          </div>

          {/* CARD 1: SWITCH UTAMA POP-UP PERINGATAN SISTEM KEAMANAN (Direct request #2) */}
          <div className="bg-white dark:bg-slate-900 border-2 border-emerald-500/30 dark:border-emerald-500/40 rounded-2xl p-5 sm:p-6 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <div className={`p-2 rounded-xl ${settings.antiCheatSecurityPopupsEnabled !== false ? 'bg-emerald-100 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}>
                  ...
                  <div>
                    ...
                    <span className="text-[11px] text-slate-500 dark:text-slate-400">
                      Mengatur apakah kotak pop-up/toast peringatan akan muncul di layar saat terdeteksi aksi mencurigakan
                    </span>
                  </div>
                </div>
              </div>

              {/* Interactive Switch Pill Toggle */}
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    Swal.fire({
                      ...
                      text: nextVal
                        ? 'Pop-up toast peringatan akan muncul jika terdeteksi aktivitas terlarang.'
                        : 'Sistem proteksi tetap bekerja di latar belakang tanpa memunculkan pop-up.',
                      showConfirmButton: false,
                      timer: 2500
                    });
                  }}
            ...
            {/* Explanation box */}
            <div className="mt-4 p-3.5 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200/80 dark:border-slate-700/80 text-xs leading-relaxed text-slate-600 dark:text-slate-300 space-y-2">
              <div className="flex items-start gap-2">
                <AlertCircle className="w-4 h-4 text-blue-600 dark:text-blue-400 shrink-0 mt-0.5" />
                <div>
                  <p className="font-semibold text-slate-800 dark:text-slate-200">
                    Panduan Pengaturan Pop-Up Keamanan:
                  </p>
                  <ul className="list-disc list-inside mt-1 space-y-1 text-slate-600 dark:text-slate-400">
                    <li>
                      <strong className="text-slate-700 dark:text-slate-300">Posisi ON:</strong> Sistem akan menampilkan toast melayang berwarna gelap bergaris amber saat terdeteksi klik kanan, penekanan F12, pintasan inspect element, atau upaya manipulasi layar.
                    </li>
                    <li>
                      <strong className="text-slate-700 dark:text-slate-300">Posisi OFF (Mode Hening):</strong> Pop-up peringatan ditiadakan sehingga tidak mengganggu antarmuka pengguna, namun proteksi tetap memblokir aksi secara aman di latar belakang.
                    </li>
                  </ul>
                </div>
              </div>
          ...
          {/* CARD 2: SWITCH MASTER SISTEM ANTI-CHEAT & ANTI REKAYASA */}
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-2">
                <div className={`p-2 rounded-xl ${settings.antiCheatEnabled !== false ? 'bg-blue-100 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-400'}`}>
                  <ShieldAlert className="w-5 h-5" />
                </div>
                <div>
                  <h4 className="text-sm font-bold text-slate-900 dark:text-white">
                    Master Shield: Sistem Anti-Cheat & Anti-Rekayasa
                  </h4>
                  <span className="text-[11px] text-slate-500 dark:text-slate-400">
                    Aktifkan atau nonaktifkan sistem proteksi peramban (browser) secara menyeluruh
                  </span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-1">
              {/* DevTools */}
              <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/80 flex items-center justify-between gap-3">
                <div className="space-y-0.5">
                  <p className="text-xs font-bold text-slate-800 dark:text-slate-200">
                    Blokir F12 / DevTools
                  </p>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400">
                    Cegah inspect element & console
                  </p>
                </div>
                <input
                  type="checkbox"
              ...
              <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/80 flex items-center justify-between gap-3">
                <div className="space-y-0.5">
                  <p className="text-xs font-bold text-slate-800 dark:text-slate-200">
                    Blokir Menu Klik Kanan
                  </p>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400">
                    Cegah menu konteks & long press
                  </p>
                </div>
                <input
                  type="checkbox"
              ...
              <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/80 flex items-center justify-between gap-3">
                <div className="space-y-0.5">
                  <p className="text-xs font-bold text-slate-800 dark:text-slate-200">
                    Proteksi Salin Teks Bebas
                  </p>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400">
                    Blokir penyalinan teks antarmuka
                  </p>
                </div>
                <input
                  type="checkbox"
          ...
          {/* CARD 3: GEOFENCE & TITIK KOORDINAT SEKOLAH */}
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="text-sm font-bold text-slate-900 dark:text-white">
                    Titik Koordinat & Geofence Lokasi Sekolah
                  </h4>
                  <span className="text-[11px] text-slate-500 dark:text-slate-400">
                    Patokan verifikasi GPS saat siswa melakukan absensi selfie mandiri
                  </span>
                </div>
              </div>
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-xl text-xs font-bold bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800 hover:bg-amber-100 dark:hover:bg-amber-900/60 transition flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2 text-xs font-mono font-semibold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2 text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
                  placeholder="200"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-1">
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">
                  Batas Jam Presensi Masuk (Terlambat):
                </label>
                <input
                  type="time"
              ...
              <div className="sm:col-span-2 space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2 text-xs font-medium text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none"
                />
              </div>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300 font-medium">
              ...
              <span>
                {syncStatus === 'saving'
                  ? 'Menyinkronkan perubahan ke server database...'
                  : 'Konfigurasi keamanan tersinkronisasi secara otomatis ke seluruh sistem.'}
              </span>
            </div>
            <button
              type="button"
